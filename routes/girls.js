...
var getNumber = function () {
    id = JSON.parse(fs.readFileSync("Jsonaux/ID_handler.json")).IDFEMALE;
};
...
var photosource = [], photosourceaux = [], skipped = [], backup=[];
var totalprofiles = 0;
var rendered = false;
var lastloaded = 0;
app.get('/', function(req, res, next) {
    ...
    getNumber();
    var albumpaths = "";
    getPaths("JsonalbumFemale", function (err, _res) {
        if (err) {
            console.log(err);
        }
        ...
        getPaths("JsonFemale", function (err, _res) {
            if (err) {
                console.log(err);
            }
